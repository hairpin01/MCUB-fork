...
# two-step approach: first skip emoji-id attribute values, then mask bare
# long numbers.
...
class KernelLogger:
    ...
    def __init__(self, kernel: "Kernel") -> None:
        ...
        # call _get_lock() before any await never race to create two locks.
    ...
    async def _send_with_retry(self, coro_factory, *, max_attempts: int = 2) -> bool:
        ...
        # loop — every exit path is already handled inside the loop body.
    ...
    async def handle_error(
        ...
        # when many unique errors accumulate over a long run.
