        ...
        try:
            ...
            if token and bot_username:
                ...
                try:
                    ...
                    self.kernel.cprint(f'=? Не удалось установить аватар: {e}', self.kernel.Colors.YELLOW)
...
init - Initialize bot (admin only)"""
                ...
                self.kernel.cprint(f'=> Токен: {self.token[:10]}...', self.kernel.Colors.CYAN)
                
                await self.start_bot()
                
            else:
                self.kernel.cprint('=X Не удалось получить данные бота из ответов BotFather', self.kernel.Colors.RED)
                self.kernel.cprint('=? Проверьте сообщения от @BotFather вручную', self.kernel.Colors.YELLOW)
    ...
    async def start_bot(self):
        ...
        try:
            ...
            self.kernel.cprint(f'=> Бот авторизован как @{self.username}', self.kernel.Colors.GREEN)
            ...
            for cmd, (pattern, handler) in self.kernel.bot_command_handlers.items():
                async def command_wrapper(event, handler=handler, cmd=cmd):
                    try:
                        ...
                        await self.kernel.handle_error(e, source=f"bot_command:{cmd}", event=event)
                
                self.bot_client.add_event_handler(
                    command_wrapper,
                    events.NewMessage(pattern=pattern, incoming=True)
                )
                self.kernel.cprint(f'=> Зарегистрирована команда бота: {pattern}', self.kernel.Colors.CYAN)
            
            self.kernel.cprint(f'=> Всего зарегистрировано команд бота: {len(self.kernel.bot_command_handlers)}', self.kernel.Colors.GREEN)
            
        except Exception as e:
            self.kernel.cprint(f'=X Ошибка регистрации команд модулей: {e}', self.kernel.Colors.RED)
