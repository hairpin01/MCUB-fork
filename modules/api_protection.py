...
def register(kernel):
    ...
    RATE_LIMITS = {
        'default': {'requests': 30, 'seconds': 60},
        'dangerous': {'requests': 5, 'seconds': 300},
        'message': {'requests': 100, 'seconds': 60},
    }
    
    def cleanup_old_requests():
        now = time.time()
        for key in list(request_timestamps.keys()):
            request_timestamps[key] = [t for t in request_timestamps[key] if now - t < 3600]
    ...
    async def enforce_cooldown(event, seconds, reason="Слишком много запросов"):
        ...
        await event.edit(f'✅ Бот разблокирован после {seconds} секунд ожидания')
    ...
    async def api_protection_handler(event):
        ...
        if len(args) > 1:
            if args[1] in ['on', 'enable', 'true']:
                ...
                await event.edit(f'❌ Использование: {kernel.custom_prefix}api_protection [on/off]')
                return
        else:
            status = '✅ включена' if protection_enabled else '❌ выключена'
            limits_info = '\n'.join([f'• {k}: {v["requests"]} запросов за {v["seconds"]} сек' for k, v in RATE_LIMITS.items()])
