...
def register(kernel):
    ...
    async def start_handler(event):
        try:
            await event.reply(
                ...
                buttons=[
                    [
                        Button.url(lang_strings['github_repo'], "https://github.com/hairpin01/MCUB-fork"),
                        Button.url(lang_strings['original_mcub'], "https://github.com/Mitrichdfklwhcluio/MCUBFB"),
                    ],
                    [
                        Button.url(lang_strings['support'], "https://t.me/+LVnbdp4DNVE5YTFi")
                    ]
                ],
            )
        except Exception as e:
    ...
    async def delete_bot_handler(event):
        try:
            ...
            kernel.logger.info(f"{lang_strings['bot_removed']} {event.chat_id} пользователем {event.sender_id}")
        ...
        try:
            ...
            if lang == "ru":
                text = (
                    ...
                    f"👉 {lang_strings['remove']} <code>.um [название]</code>\n"
                ...
                text = (
                    ...
                    f"👉 {lang_strings['remove']} <code>.um [name]</code>\n"
            ...
            if msg_id:
                try:
                    await event.edit(text, parse_mode="html",
                                     buttons=[Button.url(lang_strings['github_repo'], "https://github.com/hairpin01/MCUB-fork")])
                except Exception:
    ...
    async def ping_bot_handler(event):
        await event.reply(f"<blockquote>{lang_strings['pong']}</blockquote>", parse_mode='html')
