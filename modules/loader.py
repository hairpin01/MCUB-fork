...
def register(kernel):
    ...
    def get_module_commands(module_name):
        ...
        return list(set(commands)), aliases_info
    ...
    async def handle_catalog(event, query_or_data):
        try:
            ...
            try:
                modules = await kernel.get_repo_modules_list(repo_url)
                repo_name = await kernel.get_repo_name(repo_url)
    ...
    async def install_module_handler(event):
        if not event.is_reply:
            await event.edit(
                t("reply_to_py", warning=CUSTOM_EMOJI["warning"]), parse_mode="html"
            )
            return
        ...
        if not reply.document or not reply.document.attributes[0].file_name.endswith(
            ...
            await event.edit(
                t("not_py_file", warning=CUSTOM_EMOJI["warning"]), parse_mode="html"
            )
            return
        ...
        try:
            ...
            _me = await kernel.client.get_me()
            mcub = (
            ...
            if re.search(r"^from \.\. import", code, re.MULTILINE) or re.search(
                r"^import loader\b", code, re.MULTILINE
            ):
                add_log(t("log_incompatible"))
                await msg.edit(
                    t("modules_not_mcub", mcub=mcub, warning=CUSTOM_EMOJI["warning"]),
                    parse_mode="html",
                )
                os.remove(file_path)
                return
            ...
            if success:
                ...
                commands, aliases_info = get_module_commands(module_name)
    ...
    async def download_module_handler(event):
        ...
        if args[1] == "-list":
            if len(args) == 2:
                await event.edit(
                    t("dlm_list_loading", loading=CUSTOM_EMOJI["loading"]),
                    parse_mode="html",
                )
                ...
                await event.edit(final_msg, parse_mode="html")
                return
            else:
                module_name = args[2]
        ...
        try:
            ...
            if is_url:
                try:
                    ...
                    status, code = await kernel._repo._http_get(module_or_url)
                    if status == 200 and code:
                        add_log(t("log_download_success", status=status))
                        save_name = module_name + ".py"
                    else:
                        add_log(t("log_download_failed", status=status))
                        await msg.edit(
                            t(
                                "url_download_error",
                                warning=CUSTOM_EMOJI["warning"],
    ...
    async def unload_module_handler(event):
        ...
        await kernel.log_module(f"Модуль {module_name} удалён")
        await event.edit(
            t(
                "module_unloaded",
                success=CUSTOM_EMOJI["success"],
    ...
    async def upload_module_handler(event):
        ...
        if not os.path.exists(file_path):
            await event.edit(
                t("module_file_not_found", warning=CUSTOM_EMOJI["warning"]),
                parse_mode="html",
            )
            return
    ...
    async def reload_module_handler(event):
        ...
        if success:
            commands, aliases = get_module_commands(module_name)
            cmd_text = (
            ...
            await msg.edit(
                t("reload_error", warning=CUSTOM_EMOJI["warning"]), parse_mode="html"
            )

    @kernel.register.command("modules")
    async def modules_list_handler(event):
        await kernel.log_module(f"🔷 Просмотр списка модулей")

        if not kernel.loaded_modules and not kernel.system_modules:
            await event.edit(
                t("no_modules", folder=CUSTOM_EMOJI["folder"]), parse_mode="html"
            )
            return
        ...
        if kernel.system_modules:
            ...
            for name in sorted(kernel.system_modules.keys()):
                commands, _ = get_module_commands(name)
    ...
    async def add_repo_handler(event):
        ...
        if success:
            await event.edit(
                f'{CUSTOM_EMOJI["success"]} <b>{message}</b>', parse_mode="html"
            )
        else:
            await event.edit(
                f'{CUSTOM_EMOJI["warning"]} <b>{message}</b>', parse_mode="html"
            )
