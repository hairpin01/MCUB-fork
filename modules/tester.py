...
<blockquote>{CUSTOM_EMOJI['❄️']} <b>uptime:</b> {uptime}</blockquote>"""
            ...
            await event.edit(
                f"{CUSTOM_EMOJI['❄️']} <b>Ошибка, смотри логи</b>", parse_mode="html"
            )
    ...
    async def logs_handler(event):
        try:
            ...
            log_files = sorted(
                [f for f in os.listdir(kernel.LOGS_DIR) if f.endswith(".log")]
            )
            if not log_files:
    ...
    async def freezing_handler(event):
        try:
            ...
            if len(args) < 2:
                await event.edit(
                    f'{CUSTOM_EMOJI["🗯"]} Использование: {kernel.custom_prefix}freezing [секунды]'
                )
                return
