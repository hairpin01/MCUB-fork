...
def register(kernel):
    ...
    async def update_handler(event):
        ...
        try:
            ...
            await msg.edit(
                "🔧 <b>пробую другой метод обновления...</b>", parse_mode="html"
            )
                    ...
                    if resp.status == 200:
                        ...
                        if "VERSION" in new_code:
                            ...
                            if new_version and new_version.group(1) != kernel.VERSION:
                                emoji = random.choice(emojis)
                                await msg.edit(
                                    f"📥 <b>Обновляю до {new_version.group(1)}...</b> {emoji}",
                                    parse_mode="html",
                                )
                            ...
                            await msg.edit(
                                "❌ <b>Не удалось проверить версию</b>",
                                parse_mode="html",
                            )
                    else:
                        await msg.edit(
                            "❌ <b>Не удалось получить обновление</b>",
                            parse_mode="html",
                        )

        except Exception as e:
            await msg.edit(
                f"❌ <b>Ошибка:</b> <code>{str(e)}</code>", parse_mode="html"
            )
    ...
    async def stop_handler(event):
        ...
        emoji = random.choice(emojis)
        await event.edit(
            f"🧲 <b>Твой <i>{await mcub_handler()}</i> останавливается...</b> {emoji}",
            parse_mode="html",
        )
    ...
    async def rollback_handler(event):
        ...
        msg = await event.edit(
            "🔙 <b>Откатываю к предыдущей версии...</b> <i>{emojis}</i>",
            parse_mode="html",
        )
        ...
        try:
            ...
            await msg.edit(
                f"❌ <b>Ошибка отката:</b> <code>{str(e)}</code>", parse_mode="html"
            )
