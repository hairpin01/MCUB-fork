...
def register(client):
    ...
    @client.on(events.NewMessage(outgoing=True, pattern=r'^\.tabfix(?:\s+(.*))?$'))
    async def tabfix_handler(event):
        ...
        if args.strip() == "process":
            if not session.temp_dir or not os.path.exists(session.temp_dir):
                await event.edit("❌ Сначала активируйте пакетный режим: `.tabfix batch`")
                return
            ...
            if not files:
                await event.edit("❌ Нет файлов для обработки")
                return
        ...
        while i < len(arg_list):
            arg = arg_list[i]
            if arg == "-s" and i + 1 < len(arg_list):
                try:
                    opts["spaces"] = int(arg_list[i + 1])
                    i += 1
                except ValueError:
                    pass
            elif arg == "--json":
            ...
            elif arg == "--password" and i + 1 < len(arg_list):
                opts["password"] = arg_list[i + 1]
                session.zip_password = arg_list[i + 1]
                i += 1
            i += 1
        ...
        try:
            ...
            if is_zip:
                ...
                if not file_paths and total_in_zip > 0:
                    if opts["password"]:
                        await event.edit(
                            "❌ **Не удалось извлечь файлы из архива!**\n"
                            "Возможные причины:\n"
                            "• Неверный пароль\n"
                            "• Архив поврежден\n"
            ...
            if not file_paths:
                await event.edit("❌ **Не удалось получить файлы для обработки**")
                processing_users.remove(user_id)
                if os.path.exists(temp_dir):
                    import shutil
                    shutil.rmtree(temp_dir)
            ...
            if len(file_paths) == 1 and not opts["return_zip"]:
                ...
                if result.errors:
                    await event.edit(f"❌ **Ошибка:** `{result.errors[0]}`")
            ...
            await event.edit(f"❌ **Критическая ошибка:** `{type(e).__name__}: {str(e)[:200]}`")
        finally:
            processing_users.remove(user_id)
            if os.path.exists(temp_dir):
                import shutil
                ...
                if file_name.endswith('.zip'):
                    try:
                        ...
                        if extracted_files:
                            ...
                            await event.edit(f"❌ **Не удалось распаковать архив!**\nПроверьте пароль или целостность архива.")
                    except Exception as e:
                        await event.edit(f"❌ **Ошибка распаковки архива:** {str(e)}")
                else:
                    await event.edit(f"✅ Файл `{file_name}` сохранен\nОтправьте еще файлы или `.tabfix process`")
            except Exception as e:
                await event.edit(f"❌ Ошибка сохранения: {str(e)}")
        elif event.message.text and event.message.text.startswith('.'):
            return
        elif event.message.text:
            session.cleanup()
            await event.edit("❌ Пакетный режим отменен")

    @client.on(events.NewMessage(outgoing=True, pattern=r'^\.tabfix_stats$'))
    ...
    @client.on(events.CallbackQuery(pattern=b"clear_sessions"))
