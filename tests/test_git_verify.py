...
def pin_for(path: Path) -> kg.KeyPin:
    ...
    fingerprints = tuple(kg.ssh_fingerprint(line) for line in lines)
    return kg.KeyPin(hashlib.sha256(raw).hexdigest(), fingerprints[0], fingerprints[1:])
...
def env(tmp_path, monkeypatch):
    ...
    trusted_second = keygen(tmp_path / "trusted-second", "rich-git-second")
    ...
    e.root = repo
    e.trusted = trusted
    e.trusted_second = trusted_second
    e.other = other
    e.tmp = tmp_path
    e.n = 0
...
class TestCheckCommit:
    ...
    @pytest.mark.asyncio
    async def test_commit_signed_with_second_trusted_key_is_accepted(self, env):
        key_file = env.root / gv.TRUSTED_KEY_FILE
        key_file.write_text(
            key_file.read_text() + (env.tmp / "trusted-second.pub").read_text()
        )
        pin = pin_for(key_file)
        sha = env.commit(env.trusted_second)
    ...
    @pytest.mark.asyncio
    async def test_host_config_cannot_replace_ssh_verifier(self, env, monkeypatch):
        sha = env.commit(env.other)
        fake_verifier = env.tmp / "fake-ssh-keygen"
        fake_verifier.write_text("#!/bin/sh\nexit 0\n")
        fake_verifier.chmod(0o755)
        monkeypatch.setenv("GIT_CONFIG_COUNT", "1")
        monkeypatch.setenv("GIT_CONFIG_KEY_0", "gpg.ssh.program")
        monkeypatch.setenv("GIT_CONFIG_VALUE_0", str(fake_verifier))
        res = await check(env, sha)
        assert res.state is gv.SigState.MISMATCH
...
class TestKeyPin:
    ...
    @pytest.mark.asyncio
    async def test_key_swap_after_pin_check_cannot_change_keyring(
        self, env, monkeypatch
    ):
        key_file = env.root / gv.TRUSTED_KEY_FILE
        pin = pin_for(key_file)
        sha = env.commit(env.trusted)
        original_check = kg.check_key

        def check_then_swap(path, checked_pin):
            result = original_check(path, checked_pin)
            key_file.write_text((env.tmp / "other.pub").read_text())
            return result

        monkeypatch.setattr(kg, "check_key", check_then_swap)
        res = await check(env, sha, pin=pin)
        assert res.state is gv.SigState.VERIFIED
