...
class TestRealKey:
    def test_repo_key_matches_both_pins(self):
        ...
        assert res.actual_fingerprints == kg.EXPECTED_FINGERPRINTS
        assert res.keys == tuple(
            " ".join(line.split()[:2])
            for line in (REPO_ROOT / kg.KEY_FILE_NAME).read_text().splitlines()
    ...
    def test_pinned_values_are_the_published_ones(self):
        assert (
            kg.EXPECTED_SHA256
            == "0343d03f26bbd8a66e3db60ddd8cb7f258c9192c1b218ddf77efcdfd22ae3e53"
        )
        assert kg.EXPECTED_FINGERPRINTS == (
    ...
    def test_original_single_fingerprint_pin_api_still_works(self):
        pin = kg.KeyPin(kg.EXPECTED_SHA256, kg.EXPECTED_FINGERPRINT)
        assert pin.fingerprints == (kg.EXPECTED_FINGERPRINT,)

    @needs_ssh_keygen
    def test_pins_agree_with_ssh_keygen(self, tmp_path):
        lines = [
            line
            for line in (REPO_ROOT / kg.KEY_FILE_NAME).read_text().splitlines()
            if line.strip() and not line.startswith("#")
        ]
        assert len(lines) == len(kg.EXPECTED_FINGERPRINTS)
        for index, line in enumerate(lines):
            path = tmp_path / f"trusted-{index}.pub"
            path.write_text(line)
            out = subprocess.check_output(
                ["ssh-keygen", "-l", "-f", str(path)], text=True
            )
            assert out.split()[1] == kg.EXPECTED_FINGERPRINTS[index]
...
class TestDetection:
    ...
    def test_two_keys_in_one_file_are_accepted(self, real_key):
        res = kg.check_key(real_key)
        assert res.ok, res.problems
        assert res.actual_fingerprints == kg.EXPECTED_FINGERPRINTS

    def test_only_one_trusted_key_is_rejected(self, real_key):
        lines = real_key.read_text().splitlines()
        real_key.write_text(lines[0] + "\n")
        res = kg.check_key(real_key)
        assert not res.ok
        assert "sha256 mismatch" in res.problems
        assert "fingerprint mismatch" in res.problems
    ...
    @needs_ssh_keygen
    def test_extra_untrusted_key_is_rejected(self, real_key, tmp_path):
        other = make_key(tmp_path / "other")
    ...
    def test_duplicate_key_is_rejected(self, real_key):
        lines = real_key.read_text().splitlines()
        real_key.write_text(lines[0] + "\n" + lines[1] + "\n" + lines[1] + "\n")
        res = kg.check_key(real_key)
        assert not res.ok
    ...
    def test_malformed_second_key_rejects_whole_file(self, real_key):
        real_key.write_text(
            real_key.read_text() + "ssh-ed25519 !!!notbase64!!! attacker\n"
        )
        res = kg.check_key(real_key)
        assert not res.ok
        assert "sha256 mismatch" in res.problems
        assert any("not an OpenSSH public key line" in p for p in res.problems)

    def test_pem_block_mixed_with_keys_is_rejected(self, real_key):
        real_key.write_text(
            real_key.read_text()
            + "-----BEGIN PUBLIC KEY-----\nnot-an-openssh-key\n"
            + "-----END PUBLIC KEY-----\n"
        ...
        assert not res.ok
        assert "sha256 mismatch" in res.problems
        assert any("PGP/PEM" in p for p in res.problems)
    ...
    def test_empty_file(self, tmp_path):
        ...
        assert not res.ok
        assert "sha256 mismatch" in res.problems
        assert "no keys found" in " ".join(res.problems)
...
class TestEnforceOrExit:
    ...
    def test_prints_details_and_exits_on_mismatch(self, real_key):
        ...
        assert all(fingerprint in out for fingerprint in kg.EXPECTED_FINGERPRINTS)
