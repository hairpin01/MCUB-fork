...
* The allowed-signers list is generated from that file and handed to git with
  ``-c gpg.ssh.allowedSignersFile=...``. The SSH format and resolved
  ``ssh-keygen`` executable are pinned with ``-c gpg.format=ssh`` and
  ``-c gpg.ssh.program=...`` as well, so the host's Git/GPG configuration cannot
  replace the verifier or widen the trust.
...
def load_trusted_keys(root: str, filename: str = TRUSTED_KEY_FILE) -> list[str]:
    ...
    try:
        return key_guard.parse_open_ssh_keys(text)
    except ValueError as exc:
        raise _KeyFileError(f"{path}: {exc}") from exc
...
async def check_commit(
    ...
    *,
    ...
    ``pin`` is the SHA-256/fingerprint set the key file must match before it is
